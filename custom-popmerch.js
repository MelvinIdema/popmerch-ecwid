(function() {
  ...
  function log(msg, data = null) {
    ...
    data ? console.log(`%c[StockMgr]%c ${msg}`, style, "", data) : console.log(`%c[StockMgr]%c ${msg}`, style, "");
  }
  function logWarn(msg, data = null) {
    if (!isDebug()) return;
    const style = "background:#f57c00;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold;";
    data ? console.warn(`%c[StockMgr]%c ${msg}`, style, "", data) : console.warn(`%c[StockMgr]%c ${msg}`, style, "");
  }
  ...
  function buildAvailabilityMap(combinations) {
    ...
    for (const combo of combinations) {
      ...
      for (const opt of options) {
        ...
        if (!map.has(name)) map.set(name, /* @__PURE__ */ new Map());
